...
export default function ContactFooter() {
  ...
  const t = useT(lang);
  const contacts = [
    { icon: "MessageCircle", text: t("writeMax"), href: "https://max.ru/u/f9LHodD0cOI1V0FgyQPvD3KYqH0JhZ9FjlJOLtmC6aBl0py9u_CJcZ6G-7w" },
          ...
          <div className="grid md:grid-cols-2 gap-8 md:gap-12 text-left">
            {status === "ok" ? (
              <div className="flex flex-col items-start justify-center rounded-2xl p-8" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}>
                ...
                <h3 className="text-xl font-bold text-white mb-1.5">{t("formSentTitle")}</h3>
                <p className="text-sm mb-5" style={{ color: "#9CA3AF" }}>{t("formSentDesc")}</p>
                <button onClick={() => setStatus("idle")}
                  className="text-sm font-semibold px-5 py-2.5 rounded-full transition-all hover:bg-white/10"
                  style={{ color: "#fff", border: "1px solid rgba(255,255,255,0.2)" }}>{t("formSendAnother")}</button>
              </div>
            ) : (
              <div className="flex flex-col">
                <div className="grid sm:grid-cols-2 gap-3 mb-3">
                  <div className="relative">
                    ...
                    <input type="text" placeholder={t("phName")} value={name} onChange={(e) => setName(e.target.value)}
                ...
                <textarea placeholder={t("phMessage")} rows={4} value={message} onChange={(e) => setMessage(e.target.value)}
                ...
                <button onClick={submit} disabled={status === "sending"}
                  ...
                  {status === "sending" ? (
                    <><Icon name="LoaderCircle" size={17} className="animate-spin" /> {t("sending")}</>
                  ) : (
                    <>{t("getConsult")} <Icon name="ArrowRight" size={17} /></>
                  )}
                </button>
                {status === "error" && (
                  <p className="text-sm mt-3 flex items-center gap-1.5" style={{ color: "#f87171" }}>
                    <Icon name="CircleAlert" size={15} /> {t("formError")}
                  </p>
                )}
                <p className="text-xs mt-3" style={{ color: "#6B7280" }}>{t("formDisclaimer")}</p>
              </div>
            )}
            <div className="flex flex-col gap-2.5">
              ...
              <div className="flex items-center gap-4 mt-3 pt-5" style={{ borderTop: "1px solid rgba(255,255,255,0.1)" }}>
                ...
                <p className="text-xs leading-relaxed" style={{ color: "#9CA3AF" }}>{t("wechatHint")}</p>
              </div>
            </div>
          </div>
        </div>
